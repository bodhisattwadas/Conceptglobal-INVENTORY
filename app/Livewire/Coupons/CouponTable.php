...
final class CouponTable extends PowerGridComponent
{
    ...
    public function setUp(): array
    {
        ...
        return [
            PowerGrid::exportable('coupon_export_' . now()->format('Y_m_d'))
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ->add('id')
            ->add('code')
            ->add('type', fn(DiscountCoupon $model) => $model->type->label())
            ->add('value', fn(DiscountCoupon $model) => $model->type->value === 'percentage'
                ? $model->value . '%'
                : format_money($model->value))
            ->add('usage', fn(DiscountCoupon $model) => $model->used_count . ' / ' . ($model->usage_limit ?? '∞'))
            ->add('expires_at_formatted', fn(DiscountCoupon $model) => $model->expires_at?->format('d M Y') ?? '-')
            ->add('is_active_label', function (DiscountCoupon $model) {
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Active', 'is_active_label', 'is_active')
                ->sortable()
    ...
    public function actions(DiscountCoupon $row): array
    {
        return [
            Button::add('view')
                ...
                ->dispatch('show-coupon', ['coupon' => $row->id])
                ->tooltip('View Coupon'),
            ...
            Button::add('edit')
                ...
                ->dispatch('edit-coupon', ['coupon' => $row->id])
                ->tooltip('Edit Coupon'),
            ...
            Button::add('delete')
                ...
                ->dispatch('open-delete-modal', [
                    'component' => 'coupons.coupon-table',
                    'method' => 'delete',
                    'params' => ['rowId' => $row->id],
                    'title' => 'Delete Coupon?',
                    'description' => "Are you sure you want to delete coupon '{$row->code}'? This action cannot be undone.",
                ])
    ...
    public function delete($rowId, DiscountCouponService $service): void
    {
        $coupon = DiscountCoupon::find($rowId);

        if ($coupon) {
            try {
                $service->deleteCoupon($coupon);
                $this->dispatch('toast', message: 'Coupon deleted successfully.', type: 'success');
            } catch (\Exception $e) {
                $message = $e instanceof DiscountCouponException
                    ? $e->getMessage()
